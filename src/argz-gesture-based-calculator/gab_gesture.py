...
def draw_3d_result(frame, results, result_value):
    """
    Draw the computed result as floating '3D' text between both hands.
    Uses MediaPipe's 3D wrist landmarks for positioning and simple depth scaling.
    """
    ...
    scale = max(0.5, 2.0 - abs(mid_z) * 4.0)

    for offset in range(3):
        cv2.putText(
            frame,
            str(result_value),
            (px - 60 + offset, py + offset),
    ...
    with mp_pose.Pose(
        ...
        while True:
            ...
            # State 3: Get second number input
                ...
                # Show equation in the top-left box
                cv2.putText(
                    frame,
                ...
                # ✨ NEW: Draw 3D floating result between hands
